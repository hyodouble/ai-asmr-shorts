...
- 제목은 영어 하나로. **다국어는 설명란이 아니라 Studio의 "자막" 항목에서 처리한다** — 아래 "배포 규칙" 절을 그대로 따를 것
- 원본(영어) 설명도 반드시 채운다
...
1. **트렌드 확인** — 주 1회면 충분하다. 2026-09 기준 1등 포맷은 **글래스 커팅 ASMR**(유리 과일 자르기). 채널명 sliceverse와도 맞는다
2. **소재 결정** — 기존 영상과 안 겹치는 조합으로. 단면이 화려한 과일이 페이오프가 세다(키위 > 사과·딸기)
3. **프롬프트 3개 작성 → 사용자 승인** (1단계 규칙은 숏폼에도 그대로 적용). 템플릿: `templates/short-glass-cutting.txt`
...
- 니치: **글래스 커팅 ASMR**(유리 과일 자르기). 2026 AI ASMR 숏폼에서 가장 강한 포맷이고 채널명 sliceverse와 맞는다
- 3막으로 연속성을 만든다: 셋업(실금) → 절정(관통·파편) → 해소(단면 공개). A·B·D·E 고정, C만 바꾸는 원칙은 롱폼과 같다
- `no ringing, no bell, no chime` 를 **뺀다.** 유리의 쨍한 소리가 상품이다. `detone.py` 도 돌리지 않는다
...
- **`lowpass=f=11000` 금지.** 유리 크랙은 16 kHz까지 살아 있다. 잘라내면 쨍한 맛이 사라진다
- **`loudnorm` 으로 못 올린다.** 원본이 -30~-37 LUFS인데 크레스트가 30 dB를 넘어서 단일·2패스 모두 -22 근처에서 멈춘다. `dynaudnorm=f=150:g=15:p=0.85:m=25:s=5` 로 본체를 끌어올려야 -20까지 온다
- **`acompressor` 는 makeup 기본값이 없다.** 메이크업 없이 2단으로 걸면 신호가 -50 LUFS로 죽는다. 이 체인에서는 컴프레서를 쓰지 않는다
- **AAC 인코딩이 피크를 5 dB 밀어올린다.** 유리 파열음처럼 조밀한 트랜지언트에서 실측 -6.9 dBFS → 인코딩 후 -1.7 dBFS. 그래서 리미터 천장을 **0.45**로 잡는다. 0.89로 잡으면 인코딩 후 클리핑난다
- 워터마크 delogo 좌표(720x1280 원본): `x=574:y=1138:w=56:h=56`.
  **좌표는 이 문서 값을 우선 신뢰한다.** 다른 프로젝트 메모의 큰 박스 값을 썼다가 매크로 텍스처를 뭉갤 뻔했다(2026-09-13)
- **`loudnorm`의 `linear=true`는 게인만 선형 이동시킬 뿐 LRA를 안 줄인다.** LRA 압축이 목적이면 옵션을 빼고 dynamic 모드로 쓴다
- 최종: 1080x1920 / 24fps / -20 LUFS 근처 / 클립 간 편차 1.5 dB 이내
...
### 배포 규칙 — 다국어는 "설명"이 아니라 "자막" 항목이다

7단계에 한때 적혀 있던 `Keywords:` 한 줄 규칙은 실제 채널에서 쓰인 적이 없어서 2026-09-11에 폐기했다. 실제 방식은 이렇다.

Studio 좌측 **자막** → `언어 추가` → 언어별 **제목 및 설명** 번역을 따로 게시한다. 자막 파일(SRT)은 안 올린다 — 말이 없는 영상이라 필요 없다.

- 고정 5개 언어: **영어(미국, 원본) · 한국어 · 일본어 · 스페인어 · 포르투갈어**
- 원본(영어) 설명도 반드시 채운다. 안 채우면 영어권 시청자에게 설명이 안 보인다 (2026-09-06 쇼츠 4개가 이 상태로 올라가 있었고 9/11에 채워 넣었다)
