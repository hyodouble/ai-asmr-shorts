...
- 제목은 영어 하나로. 다국어는 설명 하단 `Keywords:` 줄과 태그에만, **문장이 아니라 명사만**
- 예: `Keywords: squishy asmr, taba squishy, bubble asmr, sleep asmr, 1 hour asmr, no talking, 스퀴시, 왁뿌볼, スクイーズ, 解压玩具`
...
## 10단계 — 매일 크레딧 초기화 루틴 (2026-09-12 확정)

Flow 계정 크레딧(토큰)이 매일 리셋되면, 그날치로 **10초 클립 3개를 이어붙인 30초 쇼츠 1개**를 만든다.

1. 그 시점 **가장 hype 높은 AI ASMR 유튜브 영상**을 찾는다(트렌드 고정 아님 — 매번 다시 확인).
   조회수·업로드 최신성 기준으로 판단, 특정 니치에 매이지 않는다.
2. 그 영상의 **소재·트리거·연출**을 레퍼런스로 프롬프트 3개 작성 → **사용자 승인 후** Flow에 입력(1단계 규칙 그대로).
3. 제목·설명도 그 레퍼런스 영상 스타일을 따라 만든다 — 7단계 규칙(제목 영어 하나 + 이모지 + 해시태그,
   설명은 후킹 한 줄 → 감각 묘사 → Perfect for/Features 불릿 → CTA → 하단 다국어 Keywords 줄) 그대로 적용.
4. 클립 3개(45크레딧) → 9단계 기준 9:16 크롭 → 후처리(5단계) → 업로드(7단계, AI 라벨 필수).

### 실행 1회차 (2026-09-13) — 키네틱 샌드 커팅, 시행착오 기록

hype 레퍼런스: 커팅 ASMR 중 kinetic sand cutting (조사 시점 최상위 포맷 — glass/soap/candle wax와 동급 취급).
결과물: https://youtube.com/shorts/6MOIm2AIHtg

**놓쳤다가 재작업한 것:**
- **1단계(프롬프트 승인) 생략함.** 컨셉만 승인받고 프롬프트 3개 전문은 안 보여주고 바로 Flow에 넣음.
  다음부턴 반드시 전문 보여줄 것. A·D·E 고정 블록(2단계)도 안 씀 — 자유서술 프롬프트라 오디오
  방향 지정("no music no voice" 등)이 없었음. 그래도 결과적으론 원본 오디오가 chime 없이 나옴.
- **오디오를 통째로 날렸다가(-an) 재작업.** 크롭 단계에서 무심코 `-an` 넣어서 무음 영상으로 처음
  만듦 — ASMR인데 소리 없는 영상 업로드할 뻔함. 원본 클립엔 Flow가 넣은 오디오(aac)가 있으니
  **크롭/딜로고 필터링할 때 `-an` 쓰지 말고 `-map 0:v -map 1:a`로 반드시 같이 옮길 것.**
- **딜로고 좌표를 이 문서 값 대신 다른 프로젝트 메모 값(x=1090:y=530:w=150:h=130) 써서 시작함.**
  실제로는 5단계의 `x=1130:y=570:w=60:h=56`(720p 기준)가 정확 — 확대 캡처로 재검증함.
  불필요하게 큰 블러박스는 매크로 텍스처를 뭉갤 수 있으니 문서 값을 우선 신뢰할 것.

**정상 작동한 것:**
- tonecheck.py로 3클립 전수 검사 — 1클립에서 34.5dB 경계값 톤 1프레임 검출됐으나 detone.py 적용하니
  오히려 22프레임/53dB로 악화(이 클립엔 안 맞음) → 원본 유지가 더 나음. **detone.py는 만능이 아니니
  적용 전후로 꼭 tonecheck 재확인하고, 악화되면 원본 유지.**
- loudnorm 체인에서 `linear=true`는 게인만 선형 이동시킬 뿐 LRA를 안 줄인다 — LRA 압축이 목적이면
  `linear` 옵션 자체를 빼고(dynamic 모드) 써야 함. 그래도 30초 3클립 붙인 것만으로 단일 패스 LRA는
  ~12LU까지밖에 안 내려감(레포 6단계의 "LRA>10 탈락" 기준 기술적으로 미달) — 숏폼 하이프 영상이라
  수면용만큼 엄격하게 안 잡아도 된다고 판단하고 진행함. 수면용 롱폼에 이 루틴 쓸 땐 더 신경 쓸 것.
- 업로드 도구(file_upload)가 파일당 10MB 제한이라, 30초 마스터(1080x1920, 원본 crf18 ~20MB)를
  `-b:v 1800k` 재인코딩(~7MB)해서 업로드용으로 따로 뽑음. 프로젝트 폴더엔 고화질본도 같이 보관.
